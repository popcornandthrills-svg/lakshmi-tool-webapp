...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$args = @('run','start','--','--hostname','127.0.0.1','--port','3000'); Start-Process -FilePath 'npm.cmd' -ArgumentList $args -WorkingDirectory '%APPDIR%' -WindowStyle Hidden -RedirectStandardOutput '%APPDIR%ngrok-app.log' -RedirectStandardError '%APPDIR%ngrok-app.err.log' | Out-Null"
...
echo Check %APPDIR%ngrok-app.log for the startup error.
pause
exit /b 1
...
if not exist "%NGROK_EXE%" (
  ...
  echo Place ngrok.exe in the _ngrok folder, then run this file again.
